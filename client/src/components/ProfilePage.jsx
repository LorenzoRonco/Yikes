...
import { Game } from "../../../server/GCModels.mjs";
...
function ProfilePage(props) {
    ...
    useEffect(() => {
        ...
        const fetchHistory = async () => {
            ...
            setError(null);
            try {
                const history = await API.getGamesHistory(props.user.id);
                setGames(history);
    ...
    return (
        <Container className="my-5">
            <Row>
                ...
                {/* Game History */}
                <Col md={8}>
                    <GameHistory setSortAsc={setSortAsc}
                        sortAsc={sortAsc}
                        loading={loading}
                        error={error}
                        games={games}
    ...
    return (
        <>
            <Card className="shadow-sm rounded-4 mb-4">
                ...
                <Card.Body>
                    <Card.Title className="mb-3">{props.user.name}</Card.Title>
                    <Card.Text>Welcome back, <strong>{props.user.name}</strong>!</Card.Text>
                </Card.Body>
            </Card>
        </>
    );
...
function GameHistory({ setSortAsc, sortAsc, loading, error, games, getStatusBadge }) {
    return (
        <>
            <Card className="shadow-sm rounded-4">
